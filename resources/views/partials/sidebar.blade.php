...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-brand">
        <div class="sidebar-brand-icon">
            ...
            @endphp
            @if ($logoUrl)
                <img src="{{ $logoUrl }}" alt="{{ $appSettings['app_name'] ?? 'Agentix' }}"
    ...
    <nav class="sidebar-nav">
        @forelse(($sidebarMenus ?? collect()) as $section => $menus)
            ...
            @foreach ($menus as $menu)
                <x-nav-item :href="$menu->destinationUrl()" :icon="$menu->icon" :active="$menu->isCurrent()" :target="$menu->target" :disabled="$menu->destinationUrl() === '#'">
                    {{ $menu->name }}
                </x-nav-item>
            @endforeach
        @empty
